...
static void CreateRenderWindow(sf::RenderWindow &window) {
    ...
    window.create(screenSize, "Chaos Equations", sf::Style::Close, settings);
...
static void CenterPlot(const std::vector<sf::Vector2f> &history) {
    ...
    float max_y = -FLT_MAX;
    for (auto i : history) {
        min_x = std::fmin(min_x, i.x);
        max_x = std::fmax(max_x, i.x);
        min_y = std::fmin(min_y, i.y);
        max_y = std::fmax(max_y, i.y);
    }
        ...
        while (window.isOpen()) {
            ...
            for (int step = 0; step < steps; ++step) {
                ...
                for (int iter = 0; iter < iters; ++iter) {
                    ...
                    const double yt = y * t;
                    const double nx =
                            xx * params[0] + yy * params[1] + tt * params[2] + xy * params[3] + xt * params[4] +
                            yt * params[5] + x * params[6] + y * params[7] + t * params[8];
                    const double ny =
                            xx * params[9] + yy * params[10] + tt * params[11] + xy * params[12] + xt * params[13] +
                            yt * params[14] + x * params[15] + y * params[16] + t * params[17];
                    x = nx;
                    y = ny;
                    ...
                    if (screenPt.x > 0.0f && screenPt.y > 0.0f && screenPt.x < window_w && screenPt.y < window_h) {
                        ...
                        const auto dist = double(500.0f * std::sqrt(dx * dx + dy * dy));
                        rolling_delta = std::min(rolling_delta,
                                                 std::max(delta / (dist + 1e-5), delta_minimum * speed_mult));
                        isOffScreen = false;
                    }
